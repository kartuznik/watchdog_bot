...
## Search query gate и relevance filter

Перед Tavily `build_search_query` мягко переформулирует тему: снимает ведущие вопросительные
слова (`какие`, `что`, `как`…) и стоп-слова, сохраняя смысловые токены. Если результат слишком
короткий или теряет значимые слова исходного запроса — в поиск уходит **полный оригинал**,
